...
void DataTreeView::dataAboutToBeUnloaded(const phantom::data& a_Old, phantom::serialization::Node*)
{
    DataTreeViewItem* pItem = getItem(a_Old);
    o_assert(pItem);
}
...
QMimeData* DataTreeView::mimeData(const QList<QTreeWidgetItem*> list) const
{
    ...
    Q_FOREACH(QTreeWidgetItem* item, list)
    {
        ...
        uint guid = pItem->getGuid();
        serialization::Node* pNode = m_pDataBase->getNode(guid);
        if(pNode == nullptr)
        {
            dataFound = true;
            phantom::data d = m_pDataBase->getData(guid);
            pItemClass = d.type()->asClass();
            o_assert(pItemClass);
            stream << (size_t)d.address() << (size_t)pItemClass;
        }
        else 
        {
            nodeFound = true;
            pItemClass = phantom::typeOf<serialization::Node>();
            stream << (size_t)pNode << (size_t)pItemClass;
        }
        ...
        pCommonClass = commonBaseClass(pCommonClass, pItemClass);
...
phantom::reflection::Class* DataTreeView::commonBaseClass( phantom::reflection::Class* a_pFirst, phantom::reflection::Class* a_pSecond ) const
{
    ...
    size_t i = 0;
    size_t count = a_pFirst->getBaseClassCount();
    for(;i<count;++i)
    {
        reflection::Class* pCommon = commonBaseClass(a_pFirst->getBaseClass(i), a_pSecond);
...
void DataTreeView::slotItemClicked(QTreeWidgetItem* item, int col)
{
    ...
    DataTreeViewItem*   pItem = static_cast<DataTreeViewItem*>(item);
    serialization::Node* pNode = m_pDataBase->getNode(pItem->getGuid());
    if(pNode == nullptr)
    {
        emit dataClicked(m_pDataBase->getData(pItem->getGuid()));
    }
    else
    {
        emit nodeClicked(pNode);
    }
}
...
void DataTreeView::slotItemDoubleClicked(QTreeWidgetItem* item, int col)
{
    ...
    if(pNode == nullptr)
    {
        emit dataDoubleClicked(m_pDataBase->getData(pItem->getGuid()));
    }
}
...
void DataTreeView::select(const phantom::vector<phantom::data>& a_Datas, const phantom::vector<serialization::Node*>& a_Nodes)
{
	...
	{
		QTreeWidgetItem* pItem = getItem(*itData);
	...
	{
		QTreeWidgetItem* pItem = getItem(*itNode);
...
void DataTreeView::defaultLoadNodeActionDelegate( DataTreeView* a_pDataTreeView, serialization::Node* a_pNode )
{
    ...
    map<string, vector<string>> missingTypes;
    ...
    else if(m_pMessage)
    {
        ...
        {
            pMessage->error(this, "Module required : %s", it->first.c_str());
...
DataTreeViewItem* DataTreeView::getItem( uint a_uiGuid ) const
{
    int i = 0;
    ...
    for(;i<count;++i)
    {
        ...
        if(pItem->getGuid() == a_uiGuid) return pItem;
        pItem = getItem(a_uiGuid, pItem);
        if(pItem) return pItem;
    }
    return NULL;
}

DataTreeViewItem* DataTreeView::getItem( uint a_uiGuid, DataTreeViewItem* a_pParentItem ) const
{
    int i = 0;
...
DataTreeViewItem* DataTreeView::getItem( const phantom::data& a_Data ) const
{
    return getItem(m_pDataBase->getGuid(a_Data));
}

DataTreeViewItem* DataTreeView::getItem( serialization::Node* a_pNode ) const
{
    return getItem(a_pNode->getGuid());
}

void DataTreeView::defaultRemoveDataActionDelegate( DataTreeView* a_pDataTreeView, const vector<uint>& a_Guids )
{
    auto pDB = a_pDataTreeView->getDataBase();
    for(auto it = a_Guids.begin(); it != a_Guids.end(); ++it)
    {
        auto d = pDB->getData(*it);
        pDB->getNode(d)->removeData(d);
    }
}
...
void DataTreeView::undoableAddData( DataTreeView* a_pDataTreeView, phantom::serialization::Node* a_pOwnerNode, phantom::reflection::Type* a_pType )
{
    ...
    AddDataCommand* pAddDataCommand = o_new(AddDataCommand)(a_pType, a_pOwnerNode);
...
bool DataTreeView::undoableLoadNode( DataTreeView* a_pDataTreeView, phantom::serialization::Node* a_pNode, UndoCommand* a_pParent )
{
    ...
    {
        phantom::Message* pMessage = m_pMessage->error(a_pDataTreeView, "Cannot load node : %s", a_pDataTreeView->getDataBase()->getNodeAttributeValue(a_pNode, "name").c_str());
        for(auto it = missingTypes.begin(); it != missingTypes.end(); ++it)
        {
            pMessage->error(this, "Module required : %s", it->first.c_str());
            auto& classes = it->second;
...
{
    m_pUndoStack->pushCommand(o_new(RemoveDataCommand)(a_pDataTreeView->getDataBase(), a_Guids));
}
