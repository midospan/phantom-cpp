...
/* *********************************************** */
o_registerN((phantom, qt, jit), LocalVariableModel);
...
namespace phantom { 
    ...
    LocalVariableModel::LocalVariableModel() 
        : m_iCallStackIndex(-1)
    {
    }

    void LocalVariableModel::addAutoProperties()
    {
            ...
            if(frameOffset == phantom::reflection::LocalVariable::e_InvalidFrameOffset) 
            {
                ...
                if(pLocalVariable == nullptr)
                {
                    ...
                    m_pAutoRoot->addSubProperty(
                        createVariableProperty(o_new(VariableNode)(variables, nullptr))
                    );
                    continue;
                }
            }
    ...
    void LocalVariableModel::setCallStackIndex( int a_iIndex )
    {
        o_assert(a_iIndex >= -1);
    ...
    void LocalVariableModel::addWatchProperty()
    {
    ...
    void LocalVariableModel::slotPropertyChanged( QtProperty* property )
    {
        if(m_pWatchRoot)
        {
    ...
    QString LocalVariableModel::valueText( const QtProperty *property ) const
    {
    ...
    void LocalVariableModel::initialize()
    {
    ...
    void LocalVariableModel::evaluateWatchProperty( QtProperty* property )
    {
        o_assert(isWatchProperty(property));
        string expression = property->propertyName().toAscii().constData();
        VariableNode* pEvaluatedVariable = evaluateExpression(expression);
    ...
    void LocalVariableModel::reevaluateWatchProperties()
    {
    ...
    void LocalVariableModel::unbindWatchProperties()
    {
        QList<QtProperty*> properties = m_pWatchRoot->subProperties();
    ...
    bool LocalVariableModel::isWatchProperty( const QtProperty* property ) const
    {
        return std::find(m_pWatchRoot->subProperties().begin(), m_pWatchRoot->subProperties().end(), property) != m_pWatchRoot->subProperties().end();
    }

    VariableNode* LocalVariableModel::evaluateExpression( const string& expression ) const
    {
        ...
        return o_new(VariableNode)(variables, nullptr);
    }

    void LocalVariableModel::saveWatchExpressions( property_tree& a_Data )
    {
    ...
    void LocalVariableModel::loadWatchExpressions( const property_tree& a_Data )
    {
