...
#define x_specialize____meta_type_base_class_solver(_meta_type_id_,...)\
    ...
    x_specialize____meta_type_base_class_solver(meta_container, o_NESTED_TYPE meta_container_class_type_of<t_Ty>::type)
        ...
        x_specialize____meta_type_base_class_solver(meta_reference_type,phantom::reflection::LValueReferenceType)
        ...
        x_specialize____meta_type_base_class_solver(meta_enum,phantom::reflection::native::TEnum<t_Ty>)
...
struct meta_class_type_of<t_Ty&, 0>
{
    typedef LValueReferenceType type;
};
...
{
    namespace_registrer(const char* a_strNamespace, const char* a_strFile);
};
...
#define o_max_type_name_of_counter_recursive_derivation 100
    ...
    namespace detail
    {
        ...
        struct type_of_builder
        {
            ...
            static bool build(meta_type** built, const string& qualifiedDecoratedName)
            {
                ...
                if(s_pType == NULL)
                {
                    ...
                    s_pType = static_cast<meta_type*>(typeByName(qualifiedDecoratedName, globalNamespace(), o_native));
                    if(s_pType == NULL)
                    {
        ...
        template<typename t_Ty>
        struct type_of_template_builder
        {
            typedef o_NESTED_TYPE meta_class_type_of<t_Ty>::type meta_type;
            static bool build(meta_type** built, const string& qualifiedDecoratedName)
            {
                ...
                if(s_pType == NULL)
                {
                    /// We check if the type hasn't been already registered through this dynamic initialization
                    /// For templates only, we allow doubles of the same instance in different module (because you want to be able to register vector<string> in two separate modules for example)
                    s_pType = static_cast<meta_type*>(dynamic_initializer()->registeredTypeByName(qualifiedDecoratedName));
                    if(s_pType == NULL)
                    {
                        s_pType = new (phantom::allocator<meta_type>::allocate(o_memory_stat_insert_arguments)) meta_type(typeNameOf<t_Ty>());
                        if(meta_type::metaType == nullptr)
        ...
        template<typename t_Ty>
        struct type_of_template_defined
        {
            typedef o_NESTED_TYPE meta_class_type_of<t_Ty>::type meta_type;
            static meta_type* object()
            {
                ...
                const type_infos& typeInfos = type_infos_of<t_Ty>::object();
                const string& qualifiedDecoratedName = typeInfos.qualified_decorated_name();
                if(type_of_template_builder<t_Ty>::build(&pType, qualifiedDecoratedName))
                {
                    dynamic_initializer()->registerType(qualifiedDecoratedName, typeInfos.scope, pType);
                    phantom::detail::module_installer_registrer<t_Ty> registrer(pType);
                }
    ...
    struct type_of<t_Ty&>
    {
        static LValueReferenceType* object()
        {
            return type_of<t_Ty>::object()->lvalueReferenceType();
        }
    };
    ...
    struct type_of<t_Ty&&>
    {
        static RValueReferenceType* object()
        {
            return type_of<t_Ty>::object()->rvalueReferenceType();
        }
    };
#endif
    template<typename t_Ty>
...
template<> inline Type* Types::get<char16_t>() { return TYPE_CHAR16_T; }
#endif
#if o_HAS_BUILT_IN_CHAR32_T == 1
template<> inline Type* Types::get<char32_t>() { return TYPE_CHAR32_T; }
#endif
...
o_specialize_fundamental_type_infos_of(char16_t);
#endif
#if o_HAS_BUILT_IN_CHAR32_T
o_specialize_fundamental_type_infos_of(char32_t);
#endif
