...
class o_jit_export JitDebuggerMutex
{
public:
    virtual void lock() = 0;
...
class o_jit_export JitDebugger
{
    enum ERequest
    {
        e_Request_None,
...
public:
    ...
    void enableMemberFunctionEntryBreakPoint(Subroutine* a_pSubroutine);
...
private:
    ...
    const vector<Instruction*>& findMemberFunctionInstructionsForLine(MemberFunction* a_pMemberFunction, int a_uiLine);
    void cleanStepOvers();
