...
#include "JitSignal.h"
#include "JitClass.h"
#include "JitInstanceAttribute.h"
/* *********************************************** */
static void* g_ptr_test = (void*)0xffffffff;
o_registerN((phantom, reflection, runtime), JitSignal);
...
JitSignal::JitSignal( const string& a_strName, Signature* a_pSignature, JitInstanceAttribute* a_pDataAttribute, bitfield a_Modifiers ) 
    : Signal(a_strName, a_pSignature, a_Modifiers) 
    , m_jit_function(nullptr)
    , m_uiDataOffset(a_pDataAttribute->getOffset())
    , m_pDataAttribute(a_pDataAttribute)
{
    o_assert(m_uiDataOffset != 0xffffffff, "Attribute must be added to the signal class before construction");
    o_assert(a_pSignature->getReturnType() == typeOf<signal_t>());
    o_assert(a_pDataAttribute->getValueType() == typeOf<connection::slot::list>());
...
//     slot*                      m_prev;           16
    ...
    un.invoke_func = &Subroutine::invoke;
    
    jit_insn_call_native(m_jit_function, "invoke", un.hacked, jit_type_create_signature(jit_abi_thiscall, jit_type_void, invoke_types, 3, 0), invoke_args, 3, 0);
