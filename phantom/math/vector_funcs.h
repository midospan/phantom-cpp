#ifndef o_vector_funcs_h__
#define o_vector_funcs_h__
...
template<typename t_Ty> 
o_forceinline t_Ty
angleTo(const vector3<t_Ty>& v1, const vector3<t_Ty>& v2) 
{ 
...
template<typename t_Ty>
o_forceinline angle<t_Ty> mul(const angle<t_Ty>& orientation, const angle<t_Ty>& r)
{
...
template<typename t_Ty>
o_forceinline angle<t_Ty> mulT(const angle<t_Ty>& orientation, const angle<t_Ty>& r)
{
    // [ qc qs] * [rc -rs] = [qc*rc+qs*rs -qc*rs+qs*rc]
    // [-qs qc]   [rs  rc]   [-qs*rc+qc*rs qs*rs+qc*rc]
    // s = qc * rs - qs * rc
...
/// Rotate a vector
template<typename t_Ty>
o_forceinline vector2<t_Ty> mul(const angle<t_Ty>& orientation, const vector2<t_Ty>& v)
{
...
template<typename t_Ty>
o_forceinline vector2<t_Ty> mulT(const angle<t_Ty>& orientation, const vector2<t_Ty>& v)
{
    return vector2<t_Ty>(orientation.c * v.x + orientation.s * v.y, -orientation.s * v.x + orientation.c * v.y);
}
...
// v2 = A.orientation.angle(B.orientation.angle(v1) + B.position) + A.position
//    = (A.orientation * B.orientation).angle(v1) + A.orientation.angle(B.position) + A.position
template<typename t_Ty>
