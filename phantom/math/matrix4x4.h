...
public:
	...
	self_type (const t_Ty entry[16], bool colMajor = true);
	...
	void computePerspectiveProjectionParameters(t_Ty aspectRatio, t_Ty fovy, t_Ty near, t_Ty& left, t_Ty& right, t_Ty& bottom, t_Ty& top) const
	{
		...
		t_Ty half_w = tanThetaX * near;
		t_Ty half_h = tanThetaY * near;
	...
	void makePerspectiveProjection(t_Ty aspectRatio, t_Ty fovy, t_Ty anear, t_Ty afar, aabb3<t_Ty>* outAABB = nullptr)
	{
		...
		{
			// Infinite far plane
			q = INFINITE_FAR_PLANE_ADJUSTMENT - 1;
			qn = anear * (INFINITE_FAR_PLANE_ADJUSTMENT - 2);
		}
		else
		{
			q = - (afar + anear) * inv_d;
	...
	void makeOrthogonalProjection(t_Ty aspectRatio, t_Ty height, t_Ty anear, t_Ty afar, aabb3<t_Ty>* outAABB = nullptr)
	{
		...
		if (afar == 0)
		{
			...
			q = - INFINITE_FAR_PLANE_ADJUSTMENT / anear;
			qn = - INFINITE_FAR_PLANE_ADJUSTMENT - 1;
		}
		else
		{
			q = - 2 * inv_d;
	...
	{
		return memcmp(other.m, m, sizeof(t_Ty)*16) == 0;
	}
