...
#define JIT_APPLY_NUM_WORD_REGS 0
...
#define JIT_APPLY_FLOATS_IN_WORD_REGS 1
#define JIT_APPLY_DOUBLES_IN_WORD_REGS 0
#define JIT_APPLY_NFLOATS_IN_WORD_REGS 0
#define JIT_APPLY_RETURN_FLOATS_AFTER 8
#define JIT_APPLY_RETURN_DOUBLES_AFTER 8
#define JIT_APPLY_RETURN_NFLOATS_AFTER 8
#define JIT_APPLY_VARARGS_ON_STACK 1
...
#define JIT_APPLY_CAN_SPLIT_LONG 1
#define JIT_APPLY_STRUCT_RETURN_IN_REG_INIT \
	{0x8B, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}
#define JIT_APPLY_MAX_STRUCT_IN_REG 8
#define JIT_APPLY_MAX_APPLY_SIZE 12
#define JIT_APPLY_X86_FASTCALL 1
#define JIT_APPLY_PARENT_FRAME_OFFSET 0
#define JIT_APPLY_RETURN_ADDRESS_OFFSET 4
#define JIT_APPLY_BROKEN_FRAME_BUILTINS 0
#define JIT_APPLY_X86_POP_STRUCT_RETURN 1
...
typedef union
{
	...
	struct { jit_ubyte pad[8]; double f_value; } double_value;
	struct { jit_ubyte pad[8]; double f_value; } nfloat_value;
...
#define jit_apply_builder_init(builder,type)	\
	do { \
		...
		jit_memset((builder)->apply_args, 0, 12); \
		(builder)->apply_args->stack_args = (unsigned char *)alloca(jit_type_get_max_arg_size((type))); \
		(builder)->stack_used = 0; \
		(builder)->word_used = 0; \
		if(jit_type_get_abi((type)) == jit_abi_fastcall) \
            (builder)->word_max = 2; \
        else if(jit_type_get_abi((type)) == jit_abi_thiscall) \
            (builder)->word_max = 1; \
		else \
			(builder)->word_max = 0; \
...
#define jit_apply_builder_add_word(builder,value) \
	do { \
		if((builder)->word_used < (builder)->word_max) \
		{ \
...
#define jit_apply_builder_add_large_inner(builder,ptr,size,align) \
	do { \
		...
		if(((builder)->word_max - (builder)->word_used) >= __num_words) \
		{ \
			jit_memcpy((builder)->apply_args->word_regs + (builder)->word_used, (ptr), (size)); \
			(builder)->word_used += __num_words; \
		} \
		else if((builder)->word_used < (builder)->word_max) \
		{ \
			unsigned int __split = ((builder)->word_max - (builder)->word_used); \
			...
			(builder)->word_used = (builder)->word_max; \
...
#define jit_apply_builder_add_long(builder,value) \
	jit_apply_builder_add_large((builder), jit_long, (value));
#define jit_apply_builder_add_ulong(builder,value) \
	jit_apply_builder_add_large((builder), jit_ulong, (value));
#define jit_apply_builder_add_float32(builder,value) \
	jit_apply_builder_add_large((builder), jit_float32, (value));
...
#define jit_apply_parser_get_long(builder,value) \
	jit_apply_parser_get_large((builder), jit_long, jit_long, (value));
#define jit_apply_parser_get_ulong(builder,value) \
	jit_apply_parser_get_large((builder), jit_ulong, jit_ulong, (value));
#define jit_apply_parser_get_float32(builder,value) \
	jit_apply_parser_get_large((builder), jit_float32, jit_float32, (value));
