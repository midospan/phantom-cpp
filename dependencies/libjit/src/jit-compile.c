...
compile_block(jit_gencode_t gen, jit_function_t func, jit_block_t block)
{
    ...
    jit_insn_t insn;
    jit_function_insn_compilation_callback_t callback = jit_function_get_insn_compilation_callback(func);
...
cache_flush(_jit_compile_t *state)
{
	...
	if(state->cache_started)
	{
		...
		jit_flush_exec(state->code_start,
			       (unsigned int)((unsigned int)state->code_end - (unsigned int)state->code_start));
#endif
	}
}
...
codegen_prepare(_jit_compile_t *state)
{
    ...
    //if(state->func->optimization_level)
	{
 ...
 */
static void
codegen_naked(_jit_compile_t *state)
{
	jit_function_t func = state->func;
	struct jit_gencode *gen = &state->gen;
	jit_block_t block;
 ...
 */
static int
compile_naked(_jit_compile_t *state, jit_function_t func)
{
	jit_exception_func handler;
	jit_jmp_buf jbuf;
	int result;
...
jit_compile(jit_function_t func)
{
	...
	if(result == JIT_RESULT_OK)
	{
        ...
        if(func->compilation_success_callback)
        {
            func->compilation_success_callback(func);
        }
...
/*@
 * @deftypefun int jit_compile_naked (jit_function_t @var{func})
 * Compile a function to its executable form without prolog and epilog.  If the function was
 * already compiled, then do nothing.  Returns zero on error.
 *
 * If an error occurs, you can use @code{jit_function_abandon} to
 * completely destroy the function.  Once the function has been compiled
...
@*/
int
jit_compile_naked(jit_function_t func)
{
	_jit_compile_t state;
	int result;
	...
	/* Compile and record the entry point */
	result = compile_naked(&state, func);
	if(result == JIT_RESULT_OK)
	{
		func->entry_point = state.code_start;
		func->is_compiled = 1;
...
/*@
 * @deftypefun int jit_compile_entry_naked (jit_function_t @var{func}, void **@var{entry_point})
 * Compile a naked function to its executable form but do not make it
 * available for invocation yet.  It may be made available later
 * with @code{jit_function_setup_entry}.
 * @end deftypefun
@*/
int
jit_compile_entry_naked(jit_function_t func, void **entry_point)
{
	_jit_compile_t state;
	int result;
...
/*@
 * @deftypefun int jit_function_compile_naked (jit_function_t @var{func})
 * Compile a naked function to its executable form.  If the function was
 * already compiled, then do nothing.  Returns zero on error.
 *
 * If an error occurs, you can use @code{jit_function_abandon} to
 * completely destroy the function.  Once the function has been compiled
...
@*/
int
jit_function_compile_naked(jit_function_t func)
{
	return (JIT_RESULT_OK == jit_compile_naked(func));
}
/*@
...
/*@
 * @deftypefun int jit_function_compile_entry_naked (jit_function_t @var{func}, void **@var{entry_point})
 * Compile a naked function to its executable form but do not make it
 * available for invocation yet.  It may be made available later
 * with @code{jit_function_setup_entry}.
 * @end deftypefun
@*/
int
jit_function_compile_entry_naked(jit_function_t func, void **entry_point)
{
	return (JIT_RESULT_OK == jit_compile_entry_naked(func, entry_point));
}
...
void *
_jit_function_compile_on_demand_naked(jit_function_t func)
{
	_jit_compile_t state;
	int result;
