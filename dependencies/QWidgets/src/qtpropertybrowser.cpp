...
{
    QSetIterator<QtProperty *> itParent(this->m_parentItems);
    while (itParent.hasNext()) {
        QtProperty *property = itParent.next();
        property->m_manager->propertyRemoved(this, property);
    }

    this->m_manager->emitPropertyDestroyed(this);

    QListIterator<QtProperty *> itChild(this->m_subItems);
    while (itChild.hasNext()) {
        QtProperty *property = itChild.next();
        property->m_parentItems.remove(this);
    }

    itParent.toFront();
    while (itParent.hasNext()) {
        QtProperty *property = itParent.next();
        property->m_subItems.removeAll(this);
    }
}
...
*/
const QList<QtProperty *>& QtProperty::subProperties() const
{
...
{
    return this->m_manager->hasValue(this);
}
...
{
    return this->m_manager->valueIcon(this);
}
...
{
    return this->m_manager->valueText(this);
}
...
{
    if (this->m_statusTip == text)
        return;

    this->m_statusTip = text;
    propertyChanged();
}
...
{
    if (this->m_whatsThis == text)
        return;

    this->m_whatsThis = text;
    propertyChanged();
}
...
{
    QtProperty *after = 0;
    if (this->m_subItems.count() > 0)
        after = this->m_subItems.last();
            ...
            QtProperty *afterProperty)
{
    ...
    m_subItems.insert(newPos, property);
    property->m_parentItems.insert(this);

    m_manager->emitPropertyInserted(property, this, properAfterProperty);
}
...
{
    m_manager->emitPropertyChanged(this);
}

void QtProperty::propertyNameChanged()
{
    m_manager->emitPropertyNameChanged(this);
}

void QtProperty::clearSubProperties()
{
    while(!m_subItems.empty())
    {
        removeSubProperty(m_subItems.back());
    }
}

////////////////////////////////

void QtAbstractPropertyManager::emitPropertyDestroyed(QtProperty *property)
{
    if (m_properties.contains(property)) {
        emit propertyDestroyed(property);
        uninitializeProperty(property);
        m_properties.remove(property);
    }
}

void QtAbstractPropertyManager::emitPropertyChanged(QtProperty *property)
{
    emit propertyChanged(property);
}

void QtAbstractPropertyManager::emitPropertyRemoved(QtProperty *property, QtProperty *parentProperty)
{
    emit propertyRemoved(property, parentProperty);
}

void QtAbstractPropertyManager::emitPropertyInserted(QtProperty *property, QtProperty *parentProperty, QtProperty *afterProperty)
{
    emit propertyInserted(property, parentProperty, afterProperty);
}
...
QtProperty *QtAbstractPropertyManager::addProperty(const QString &name)
{
    ...
    if (property) {
        ...
        this->m_properties.insert(property);
...
void QtAbstractPropertyManager::emitPropertyNameChanged( QtProperty *property )
{
    emit propertyNameChanged(property);
}
...
void QtBrowserItem::addChild(QtBrowserItem *index, QtBrowserItem *after)
{
...
void QtBrowserItem::removeChild(QtBrowserItem *index)
{
...
void QtAbstractPropertyBrowser::insertSubTree(QtProperty *property,
            QtProperty *parentProperty)
{
    ...
    if (m_managerToProperties[manager].isEmpty()) {
        ...
        connect(manager, SIGNAL(propertyInserted(QtProperty *,
                            QtProperty *, QtProperty *)),
                this, SLOT(slotPropertyInserted(QtProperty *,
                            QtProperty *, QtProperty *)));
        connect(manager, SIGNAL(propertyRemoved(QtProperty *,
                            QtProperty *)),
        ...
        connect(manager, SIGNAL(propertyChanged(QtProperty*)),
                this, SLOT(slotPropertyDataChanged(QtProperty*)));
    }
...
void QtAbstractPropertyBrowser::removeSubTree(QtProperty *property,
            QtProperty *parentProperty)
{
    if (!m_propertyToParents.contains(property)) {
        // ASSERT
    ...
    if (m_managerToProperties[manager].isEmpty()) {
        ...
        disconnect(manager, SIGNAL(propertyInserted(QtProperty *,
                            QtProperty *, QtProperty *)),
                this, SLOT(slotPropertyInserted(QtProperty *,
                            QtProperty *, QtProperty *)));
        disconnect(manager, SIGNAL(propertyRemoved(QtProperty *,
                            QtProperty *)),
                this, SLOT(slotPropertyRemoved(QtProperty*,QtProperty*)));
...
void QtAbstractPropertyBrowser::createBrowserIndexes(QtProperty *property, QtProperty *parentProperty, QtProperty *afterProperty)
{
...
QtBrowserItem *QtAbstractPropertyBrowser::createBrowserIndex(QtProperty *property,
        QtBrowserItem *parentIndex, QtBrowserItem *afterIndex)
{
    QtBrowserItem *newIndex = new QtBrowserItem(this, property, parentIndex);
    if (parentIndex) {
    ...
    itemInserted(newIndex, afterIndex);
...
void QtAbstractPropertyBrowser::removeBrowserIndexes(QtProperty *property, QtProperty *parentProperty)
{
...
void QtAbstractPropertyBrowser::removeBrowserIndex(QtBrowserItem *index)
{
...
void QtAbstractPropertyBrowser::clearIndex(QtBrowserItem *index)
{
    QList<QtBrowserItem *> children = index->children();
...
void QtAbstractPropertyBrowser::slotPropertyInserted(QtProperty *property,
        QtProperty *parentProperty, QtProperty *afterProperty)
{
    ...
    //propertyInserted(property, parentProperty, afterProperty);
}

void QtAbstractPropertyBrowser::slotPropertyRemoved(QtProperty *property,
        QtProperty *parentProperty)
{
    if (!m_propertyToParents.contains(parentProperty))
        return;
    removeSubTree(property, parentProperty); // this line should be probably moved down after propertyRemoved call
    //propertyRemoved(property, parentProperty);
    removeBrowserIndexes(property, parentProperty);
}

void QtAbstractPropertyBrowser::slotPropertyDestroyed(QtProperty *property)
{
...
void QtAbstractPropertyBrowser::slotPropertyDataChanged(QtProperty *property)
{
    if (!m_propertyToParents.contains(property))
        return;
...
{
    return this->m_propertyToIndexes.value(property);
}
...
{
    return this->m_topLevelPropertyToIndex.value(property);
}
...
QtBrowserItem *QtAbstractPropertyBrowser::addProperty(QtProperty *property)
{
    ...
    if (this->m_subItems.count() > 0)
        afterProperty = this->m_subItems.last();
            ...
            QtProperty *afterProperty)
{
    ...
    this->createBrowserIndexes(property, 0, afterProperty);

    // traverse inserted subtree and connect to manager's signals
    this->insertSubTree(property, 0);

    this->m_subItems.insert(newPos, property);
...
void QtAbstractPropertyBrowser::removeProperty(QtProperty *property)
{
    ...
    while (pos < pendingList.count()) {
        if (pendingList.at(pos) == property) {
            this->m_subItems.removeAt(pos); //perhaps this two lines
            this->removeSubTree(property, 0); //should be moved down after propertyRemoved call.
            //propertyRemoved(property, 0);

            this->removeBrowserIndexes(property, 0);
...
{
    QtBrowserItem *oldItem = this->m_currentItem;
