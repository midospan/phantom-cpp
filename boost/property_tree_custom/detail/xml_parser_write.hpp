...
// ----------------------------------------------------------------------------
#ifndef BOOST_PROPERTY_TREE_CUSTOM_DETAIL_XML_PARSER_WRITE_HPP_INCLUDED
#define BOOST_PROPERTY_TREE_CUSTOM_DETAIL_XML_PARSER_WRITE_HPP_INCLUDED
...
namespace boost { namespace property_tree_custom { namespace xml_parser
{
    ...
    {
        stream << Str(indent * settings.indent_count, settings.indent_char);
    }
                        ...
                        )
    {
        ...
        if (separate_line)
            stream << typename Str::value_type('\n');
    }

    template<class Ptree>
    void write_xml_element(typename Ptree::key_basic_ostream &stream,
                           const typename Ptree::key_basic_string  &key,
                           const Ptree &pt,
                           int indent,
						   const xml_writer_settings<typename Ptree::key_basic_string> & settings)
    {
        ...
        else    // Nonempty key
        {
                ...
                if (optional<const Ptree &> attribs = pt.get_child_optional(xmlattr<Str>()))
                    for (It it = attribs.get().begin(); it != attribs.get().end(); ++it)
                        stream << Ch(' ') << it->first << Ch('=') <<
                                  Ch('"') << it->second.BOOST_NESTED_TEMPLATE get_value<typename Ptree::BOOST_NESTED_TEMPLATE for_char<Ch>::basic_string >() << Ch('"');
            ...
            if (!pt.data().empty())
                write_xml_text(stream,
                    pt.BOOST_NESTED_TEMPLATE get_value<typename Ptree::BOOST_NESTED_TEMPLATE for_char<Ch>::basic_string >(),
            ...
            for (It it = pt.begin(); it != pt.end(); ++it)
            {
                if (it->first == xmlattr<Str>())
                    ...
                    write_xml_comment(stream,
                        it->second.BOOST_NESTED_TEMPLATE get_value<typename Ptree::BOOST_NESTED_TEMPLATE for_char<Ch>::basic_string >(),
    ...
    template<class Ptree>
    void write_xml_internal(typename Ptree::key_basic_ostream &stream,
                            const Ptree &pt,
                            ...
                            const xml_writer_settings<typename Ptree::key_basic_string> & settings)
    {
        ...
        if (!stream)
            BOOST_PROPERTY_TREE_CUSTOM_THROW(xml_parser_error<typename Ptree::string_type>("write error", filename, 0));
    }
