...
// ----------------------------------------------------------------------------
#ifndef BOOST_PROPERTY_TREE_CUSTOM_DETAIL_XML_PARSER_UTILS_HPP_INCLUDED
#define BOOST_PROPERTY_TREE_CUSTOM_DETAIL_XML_PARSER_UTILS_HPP_INCLUDED
    ...
    Str decode_char_entities(const Str &s)
    {
        ...
        for (typename Str::const_iterator it = s.begin(); it != end; ++it)
        {
            if (*it == Ch('&'))
            {
                ...
                if (semicolon == end)
                    BOOST_PROPERTY_TREE_CUSTOM_THROW(xml_parser_error<Str>("invalid character entity", "", 0));
