...
// ----------------------------------------------------------------------------
#ifndef BOOST_PROPERTY_TREE_CUSTOM_INI_PARSER_HPP_INCLUDED
#define BOOST_PROPERTY_TREE_CUSTOM_INI_PARSER_HPP_INCLUDED
...
namespace boost { namespace property_tree_custom { namespace ini_parser
{
    ...
    class ini_parser_error: public file_parser_error<Str>
    {
    public:
        /**
         ...
         *             encountered.
         */
        ini_parser_error(const Str& message,
                         const Str& filename,
                         unsigned long line)
    ...
    void read_ini(typename Ptree::key_basic_istream &stream,
                  Ptree &pt)
    {
        ...
        while (stream.good())
        {
            ...
            if (!stream.good() && !stream.eof())
                BOOST_PROPERTY_TREE_CUSTOM_THROW(ini_parser_error(
                    "read error", "", line_no));

            // If line is non-empty
            line = property_tree_custom::detail::trim(line, stream.getloc());
            if (!line.empty())
            {
                ...
                else if (line[0] == lbracket)
                {
                    ...
                    if (end == Str::npos)
                        BOOST_PROPERTY_TREE_CUSTOM_THROW(ini_parser_error(
                            "unmatched '['", "", line_no));
                    Str key = property_tree_custom::detail::trim(
                ...
                else
                {
                    ...
                    Str data = property_tree_custom::detail::trim(
     ...
     */
    template<class Ptree>
    void read_ini(const typename Ptree::string_type &filename, 
                  Ptree &pt,
     ...
     */
    template<class Ptree>
    void write_ini(   typename Ptree::key_basic_ostream
                   > &stream,
                   const Ptree &pt,
                   int flags = 0)
    {
     ...
     */
    template<class Ptree>
	void write_ini(const typename Ptree::string_type &filename,
                   const Ptree &pt,
                   int flags = 0,
                   const std::locale &loc = std::locale())
    {
