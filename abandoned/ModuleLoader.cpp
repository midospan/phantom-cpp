...
bool ModuleLoader::loadLibrary( const string& a_strPath, phantom::Message* a_pMessage /*= nullptr*/ )
{
    ...
    Module* pModule = phantom::moduleByFileName(strPath);
    string moduleName = strPath.substr(strPath.find_last_of("/\\")+1);
    Message* pMessageLoadFailed = nullptr;

    bool result = true;
    if(LoadLibrary(ModuleLoader_makeWindowsPathString(strPath).c_str()))
    {
...
{
    string strPath = ModuleLoader_normalizePathString(a_strPath);
    o_assert(isLibraryLoaded(strPath));
    return m_LibraryModules.find(strPath)->second.begin();
}
...
map<Module*, size_t>::const_iterator ModuleLoader::endLoadedLibraryModules( const string& a_strPath ) const
{
    string strPath = ModuleLoader_normalizePathString(a_strPath);
    o_assert(isLibraryLoaded(strPath));
    return m_LibraryModules.find(strPath)->second.end();
}
...
{
    return m_LibraryModules.find(ModuleLoader_normalizePathString(a_strPath)) != m_LibraryModules.end();
}
